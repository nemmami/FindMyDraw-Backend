...
router.post("/:nbRound/:nbPlayers", authorize, async function (req, res) {
  ...
  const room = roomModel.createRoom(req.params.nbRound, req.params.nbPlayers);
...
router.put("/updateRoomId/:username", function (req, res) {
  if (!req.body || req.body.username) return res.status(400).end();
...
router.put("/updateNbRound/:id", function (req, res) {
  if (!req.body || req.body.id) return res.status(400).end();
